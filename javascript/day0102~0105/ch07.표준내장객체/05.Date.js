...
function myDateTime(date) {
    let now = new Date();
    return `${date.getFullYear()}-$twoDlight(date.getMonth()+1)}-${twoDlight(date.getDate())}` +
    `${twoDlight(date.getHours())}:${twoDlight(date.getMiutes())}:${twoDligt(date.getSeconds)}` ;
}
...
// 더하기, 빼기 
let date = new Date()
date.setDate(date.getDate() + 100 );             // 오늘부터 100일 후 카운트
console.log(myDateTime(date));     
date.setDate(date.getDate() - 100 );            // 100일 후부터 100일전, 즉 오늘.
console.log(myDateTime(date));
...
let diffMs = gradDay.getTime() - new Date().today();
