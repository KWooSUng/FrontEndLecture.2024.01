...
// 1. 국가 표준 : EUC-KR, ANSI, MS-Windows - 2바이트 완성형 코드
                                        //- ㄱ ~ ㅎ
                                        //- ㅏ ~ ㅣ
                                        //- 가 ~ 힣

// 2. web 표준 : UTF-8 - 초성 + 중성 + 종성 각 1 바이트씩 3바이트 조합형
...
console.log(hello.indexOf('하'));               // 2 (0부터 시작)
console.log('pineapple'.lastIndexOf('p'));      // 6 (0부터 시작)
...
// 날짜 추출 : 2024-01-08
console.log(today.substring(0, 10)); // 숫자 '미만'까지 (이하 X)
console.log(today.substring(0, today.indexOf('T'))); 
console.log(today.split('T')[0]);

// 시간 추출 : 02:06:00
console.log(today.substring(11, 19)); // 첫 숫자 + 숫자길이만큼 = 뒷 숫자
console.log(today.substring(today.indexOf('T')+1, today.indexOf('.'))); 
console.log(new Date().toLocaleDateString());           // 2024. 1. 8 오전 11:16:40

//YYYY-NN-DD hh:mm:ss 형식으로 만들어주는 함수
function twoDligt(num) {
    return (num < 10) ? '0' + num : string(num);        //4 -> '04' , 23 -> '23'
}
function myDateTime() {
    let now = new Date();
    return `${now.getFullYear()}-$twoDlight(now.getMonth()+1)}-${twoDlight(now.getDate())}` +
    `${twoDlight(now.getHours())}:${twoDlight(now.getMiutes())}:${twoDligt(now.getSeconds)}` ;
}
...
console.log(sample.replace('o', "O"));          // 앞에 있는 것 한 개만 바뀜

// 정규 표현식 (딥 러닝을 이용한 자연어 처리 입문 > 02-05 정규 표현식 참고)
console.log(sample.replace(/o/g, 'O'));         // 모든 o가 O로 바뀜

const str = 'A qulck browm fox - 3 - 갈색의 재빠른 여우 the lazy dog = 5 = 게으른 개';
let eng = str.replace(/[^A-Z a-z]/g, '');            // ^ -> not, 영문자와 공백만 남기고 모두 지움
...
// 문자열 찾기
console.log(hello.indexOf('안녕'));         // 결과값이 0 이상이면 찾으려고 하는 문자열이 있음
let search = 'hello'
